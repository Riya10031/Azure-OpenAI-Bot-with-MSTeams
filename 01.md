...
1. On the OpenAI **Overview** page, click on **Go to Foundry portal** button .

   ![](Images/im-03.png)

1. In the **Microsoft Foundry**, select **Deployments (1)** under Shared resources ,click on **+ Deploy model (2)** and select **Deploy base model (3)**.
...
1. On the Deploy model text-embedding-ada-002 pop-up enter the deployment name as **ada002 (1)** and scale down the **Tokens per Minute Rate Limit (thousands) (2)** to **11k** then click on **Deploy (3)**.
   ...
   ![](<Images/deploy-1(1).png>)

1. Now again Click on **+ Deploy model** from Deployments and select **Deploy base model**.

   ![](Images/im-04.png)

1. On the Select a model pop-up, Select **gpt-4.1-mini (1)** and click on **Confirm (2)**.

   ![](Images/im-06.png)

1. On the Deploy model gpt-4.1-mini pop-up , enter the deployment name as **davinci003 (1)**, deployment type as **Standard (2)** and then click on **Deploy (3)**.
...
1. Navigate to the **notebooks** folder, open **create-services.py** file from the path **Users/odl*user*<inject key="DeploymentID" enableCopy="false"/>/artifacts/openai-model**

   ![](Images/Task1-Ex3-Step1.png)

1. Select on the **Tab (1)** and click on **File (2)** and select **Save and run script in terminal (3)**.

   ![](<Images/Task1-Ex3-Step2(1).png>)

   > **Info:** This will open a new terminal window and will run the file.

1. Navigate to the **notebooks** folder, open **dataextraction.py** file from the path **Users/odl*user*<inject key="DeploymentID" enableCopy="false"/>/artifacts/openai-model**

   ![](Images/dataextraction1.png)

1. In the **dataextraction.py** file, press **Ctrl+F** and replace the `gpt-35-turbo-instruct` model name with `gpt-4-mini`. 

1. Select on the **Tab** **(1)** and click on **File (2)** and select **Save and run script in terminal (3)**.

   ![](<Images/saveandrun(1).png>)

   > **Info:** This will open a new terminal window and will run the file.

1. Navigate back to the **openai-model** folder, select **faq.ipynb (1)** file and select the kernal as **Python 3.10-AzureML (2)**.

   ![](<Images/faq2(1).png>)

1. In the first cell, add the lines of code shown below before running the cell.:
   ...
   >**Note:** Replace the <API-Key> with the key you copied in the previous step. It should look like the screenshot shown below.
